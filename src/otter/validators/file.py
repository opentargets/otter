...
async def async_exists(
    ...
    try:
        await AsyncStorageHandle(location, config=config, force_local=force_local).stat()
...
async def async_size(
    ...
    try:
        one_handle = AsyncStorageHandle(one, config=config)
        ...
        two_handle = AsyncStorageHandle(two, config=config, force_local=force_local)
