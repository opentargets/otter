...
class Spec(BaseModel, extra='allow'):
    ...
    def task_type(self) -> str:
        """The task type. Determined by the first word in the task name."""
...
"""States on which a task can be considered as ready for execution (run, validate)."""
...
class TaskContext:
    ...
    def add_specs(self, specs: Sequence[Spec]) -> None:
        """Add generated specs to the context."""
...
class Task(TaskReporter):
    ...
    `Task` is the main building block for a `Step`. Tasks are in charge of running
    the various operations that are to complete Steps.

    The config for a `Task` is contained in a :py:class:`otter.task.model.Spec` object.
    ...
    2. Subclass `Task` and implement the `run` and, optionally, `validate` methods.

    :py:attr:`spec` is the `Spec` object that contains the config model.
    :py:attr:`scratchpad` is the `Scratchpad` to replace sentinels in the config.
    """
